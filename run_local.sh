...
if [[ $# -gt 1 ]]; then
  echo "Usage:"
  echo "  ./run_local.sh"
  echo "  ./run_local.sh ./path-to-repo"
  exit 1
fi
...
if [[ "${STRICT_LOCAL_ONLY}" != "1" ]]; then
  echo "❌ STRICT_LOCAL_ONLY must be 1 for ./run_local.sh."
  echo "Run python3 main.py directly only if you intentionally allow remote endpoints."
  exit 1
fi
...
export STRICT_TRACEABILITY="${STRICT_TRACEABILITY:-0}"
...
case "${LLM_BACKEND}" in
  ...
  openai_compatible|openai-compatible|openai)
    ...
    BACKEND_MODEL="${OPENAI_COMPATIBLE_MODEL}"
...
if [[ "${LLM_BACKEND}" == "ollama" ]]; then
  echo "[INFO] ollama_model=${BACKEND_MODEL}"
  ...
  echo "[INFO] openai_compatible_model=${BACKEND_MODEL}"
fi
...
echo "[INFO] Setup OK. Running analyzer..."
if [[ $# -eq 1 ]]; then
  REPO_ARG="$1"
  echo "[INFO] repo_path=${REPO_ARG}"
  python3 main.py --repo "${REPO_ARG}"
else
  python3 main.py
fi
